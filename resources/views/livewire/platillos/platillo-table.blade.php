    ...
    <x-dialog-modal wire:model="showModal" id="platilloModal" maxWidth="2xl">
        ...
        <x-slot name="content">
            ...
            <form>
                ...
                @if ($imagen)
                    <img src="{{ $imagen->temporaryUrl() }}" alt="Vista previa" class="w-16 h-16 mt-2 rounded">
                ...
                <x-dropdown width="full" wire:model.defer="sucursal_id" dropdownClasses="mt-2">
                    <x-slot name="trigger">
                        ...
                        <x-input id="sucursal_input" type="text" value="{{ $sucursales->find($sucursal_id)->nombre ?? 'Seleccione una sucursal' }}" readonly class="block w-full mt-1 cursor-pointer" />
                    </x-slot>
                    <x-slot name="content">
